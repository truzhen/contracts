...
| `readmodels/` | ReadModelEnvelope、MobilePairingBootstrapRequest / Candidate、MobileSessionIssueIntent。 | 不持有真相源，不实现前端状态管理、PC 审批、会话签发或凭据保存。 |
...
> 2026-07-17（v0.13.0，未发布）：新增移动首配对请求、候选投影与会话签发意图三件套。均为加性 schema；显式排除 bootstrap proof、bearer、OwnerDecision 与 Receipt 真相。client 本轮 vendor/codegen，OS 保持 Host-owned 会话与 PC 审批，待 tag 后改为直接 module import。
...
| `readmodels/` | 约 60 | ReadModelEnvelope、移动首配对请求 / 候选 / 签发意图 | 🔧 其它具体投影待真实消费方驱动契约化 |
...
| 根级 | 全仓 33 个 `*.schema.json`；其中 30 个由 `embed.go` 暴露，3 个 scene schema 未 embed | schema 嵌入 + 版本漂移门禁（`check-version-drift.sh`）+ 破坏性变更与 Go↔Schema 配对门禁（`contracts-check.sh`，type 变更一律判 breaking，Owner 2026-07-10 R-a 裁定） | ✅ |
