...
> 2026-07-10（v0.9.0）：`base.OwnerDelegationGrant` 兼容新增可选 `scope.execution_scope` 与 `DelegationSubject.execution`，用于代码执行委托边界；旧 JSON 不出现这些字段时语义不变，不默认授权执行。兼容说明见 `docs/compatibility/codex-hands-delegation-execution-scope-v0.9.0.md`。
