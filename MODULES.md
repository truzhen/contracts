...
> 2026-07-16（v0.13.0 契约已定、未发布）：`pack-install-result.schema.json` 兼容新增可选 `decision_ref/run_id/nonce/artifact_sha256/gateway_proxy_required/no_raw_secret/status`。真实消费方为 truzhenos F10 市场安装 completion Receipt 与 client MarketPage；共享 DTO 保持字段可选兼容旧消费方，新的 OS HTTP success operation 将不可变 proof 设为必填。未打 tag、未发布。
