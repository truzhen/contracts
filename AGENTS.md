...
5. **禁止内容** — 不引入 DB / 网络 / 文件 I/O / 并发运行时 / provider 调用 / 真实执行；不 import `github.com/lights314/truzhenos`、`github.com/truzhen/packs`、`github.com/truzhen/truzhen-cloud` 或 client / provider 实现仓；不把 Pack 数据、前端源码、安装脚本、provider registry、raw secret、token、terminal_sn、激活码放进本仓；不无意识删字段、改必填、改语义。
6. **验证入口** — `go build ./... && go test ./... && go vet ./...`；反向依赖检查；全部 schema JSON 解析检查。修改 schema / embed 时必须同时检查 `embed.go` 覆盖关系。
7. **必须回 Owner** — 任何破坏性契约变更、SemVer 版本策略变化、新增 / 删除子包、新增 / 删除 schema、schema 必填字段变化、跨仓边界或依赖方向调整、`git push` / tag / release。
...
**「Truzhen v4 版本」统称（2026-07-11 Owner 裁定）**：上表六仓集合（`truzhenos` / `truzhen-contracts` / `truzhen-packs` / `truzhen-software` / `truzhen-cloud` / `truzhen-client-web-desktop`）后期表述统称「Truzhen v4 版本」/「v4」。这是六仓当前主线形态的集合代称，不是某单仓版本号；指具体某仓仍用标准仓名，封棺仓 `truzhenv3` 不属于 v4。权威定义见 truzhenos `NAMING_STANDARD.md`。
...
- **优化优先于新增（2026-07-11 Owner 裁定，v4 六仓统一）**：能不新增 / 不新写代码就不新写，优先在原有代码上优化调整；能不新增 / 不新写模块 / 子包就不新写，优先在原有结构上优化调整。新增类型 / 文件 / 子包前先问：现有契约能否复用、扩展或收敛达成同样目的？（本仓体量红线仍按 §11：顶层契约包非测试代码超 5000 行即视为实现逻辑漏进契约层的信号。）确需新写导致某模块超 2 万行时，按「内部子模块各 ≤ 2 万行」做功能拆分（历史已超模块 grandfather；体量语义权威源 = truzhenos `scripts/check-module-size.sh` + 其 AGENTS.md 硬纪律 29/32）。
...
- 只读消费方清点和兼容审阅可以并行；同一仓库 / worktree 同时只有一个写入者。Go 类型、JSON Schema、`VERSION` 和 embed 由主执行节点统一落盘，审阅节点不得顺手修改。
- 执行者自报不算验收；独立复核使用裁剪后的新上下文并亲自重跑证据。环境不允许独立 Agent 时使用等价的全新上下文对抗复核，不伪称独立 Agent 已验收。
- 每个修复回边必须设最大次数；同一错误连续两次仍未定位立即停止。验收通过、预算耗尽、需要新增授权、范围漂移或外部状态阻断时结束并报告；破坏性契约变化仍必须回 Owner。
...
- 本仓治理语境的「模块」指 `MODULES.md` 登记的顶层契约包（`base` / `candidates` / `gates` / `receipts` / `registry` / `readmodels` / `spines` / `market` / `monitoring` / `secrets` 等），不是 `go.mod` 声明的 Go module（整仓级 `github.com/truzhen/contracts`）；Go 语言意义上的 package 必须写全称「Go 包 / package」，不得简称「模块」。Owner 指令中的「模块」默认按顶层契约包解释，有歧义先问。跨仓提及编号业务模块要带仓前缀 os-NN / cloud-NN（本仓无编号模块，但引用它仓时同规矩）。
- 单个顶层契约包核心代码（非测试 `.go`）硬帽 **20000 行**。契约层以数据形状为业、体量天然极小（2026-07-03 全仓仅 29 个 `.go`）；本条不配守卫脚本，靠条款 + Owner 审阅守住。若未来任何顶层包非测试代码超过 **5000 行**，先视为「实现逻辑漏进契约层」的信号，按 §4 契约变更纪律回 Owner 裁定，而不是当作自然增长。
- 拆分进度必须用三态汇报：未拆 / 子包化（仍在原树，不算拆完）/ 已迁出（拆分完成）。
